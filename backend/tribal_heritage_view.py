# -*- coding: utf-8 -*-
"""
View renderer for AP & Telangana Tribal Heritage, PVTGs & PESA Hub.
"""
import json
try:
    from tribal_heritage_data import TRIBAL_HERITAGE_DATA
except ImportError:
    from backend.tribal_heritage_data import TRIBAL_HERITAGE_DATA

def render_tribal_heritage_html():
    data = TRIBAL_HERITAGE_DATA
    data_json = json.dumps(data, ensure_ascii=False)

    sections_html = ""
    for sec in data["sections"]:
        points_html = "".join([f'<li class="mb-3 text-slate-700 leading-relaxed"><i class="fa-solid fa-tree text-emerald-600 mr-2 text-sm"></i>{p}</li>' for p in sec["points"]])
        sections_html += f"""
        <div class="category-card bg-white rounded-2xl p-6 sm:p-7 shadow-sm border border-slate-200/90 mb-6 hover:shadow-md transition-all" data-category="{sec['id']}">
...
<head>
    ...
    <title>గిరిజన సంస్కృతి, PVTGs & PESA చట్టం | లక్ష్య APPSC & TSPSC</title>
...
<body class="text-slate-800 min-h-screen">
    <!-- Top Header -->
    <header class="bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-950 text-white shadow-lg sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex flex-wrap justify-between items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <div class="w-10 h-10 rounded-xl bg-emerald-500/20 border border-emerald-400/30 flex items-center justify-center text-emerald-300 text-xl">
                    <i class="fa-solid fa-feather"></i>
                </div>
                <div>
                    <h1 class="text-lg sm:text-2xl font-black tracking-tight">{data["title"]}</h1>
                    <p class="text-xs text-emerald-200">AP & TS PVTGs, మేడారం జాతర, గుస్సాడి & ధింసా, 5వ షెడ్యూల్, PESA 1996 | 30 MCQs</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="/static/pdfs/ap_ts_tribal_heritage_pesa_handbook.pdf" download class="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-sm flex items-center gap-2 shadow-lg shadow-emerald-500/20 transition-all">
    ...
    <main class="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        <!-- Overview Banner -->
        <div class="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-200 rounded-3xl p-6 sm:p-8 mb-8 shadow-sm">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-200 text-emerald-950 mb-2">
                        <i class="fa-solid fa-mountain"></i> TRIBAL HERITAGE & CONSTITUTIONAL ACTS
                    </span>
                    <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-900 mb-2">ఆంధ్రప్రదేశ్ & తెలంగాణ గిరిజన అధ్యయనాలు</h2>
                    <p class="text-slate-700 max-w-3xl leading-relaxed text-sm sm:text-base">
                        {data["overview"]["summary"]}
                    </p>
                </div>
                <div class="bg-white px-5 py-4 rounded-2xl border border-emerald-200 text-center shadow-sm shrink-0">
                    <div class="text-2xl font-black text-emerald-800">30 MCQs</div>
        ...
        <div id="controls-bar" class="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 mb-6">
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
                ...
                <button onclick="switchTab('sec_pvtgs_ap_ts')" class="tab-btn px-4 py-2 rounded-xl text-sm font-bold bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 transition-all shrink-0" data-target="sec_pvtgs_ap_ts">
                    7 PVTGs ప్రొఫైల్
                </button>
                <button onclick="switchTab('sec_jatras_festivals')" class="tab-btn px-4 py-2 rounded-xl text-sm font-bold bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 transition-all shrink-0" data-target="sec_jatras_festivals">
                    జాతరలు & పండుగలు
                </button>
                <button onclick="switchTab('sec_dances_arts')" class="tab-btn px-4 py-2 rounded-xl text-sm font-bold bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 transition-all shrink-0" data-target="sec_dances_arts">
                    నృత్యాలు & లలిత కళలు
                </button>
                <button onclick="switchTab('sec_5th_schedule_safeguards')" class="tab-btn px-4 py-2 rounded-xl text-sm font-bold bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 transition-all shrink-0" data-target="sec_5th_schedule_safeguards">
                    5వ షెడ్యూల్ & సమత తీర్పు
                </button>
                <button onclick="switchTab('sec_pesa_fra_acts')" class="tab-btn px-4 py-2 rounded-xl text-sm font-bold bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 transition-all shrink-0" data-target="sec_pesa_fra_acts">
                    PESA & FRA చట్టాలు
                </button>
                <button onclick="switchTab('tab_mcqs')" class="tab-btn px-4 py-2 rounded-xl text-sm font-bold bg-emerald-100 text-emerald-900 hover:bg-emerald-200 border border-emerald-300 transition-all shrink-0" data-target="tab_mcqs">
                    🎯 30 ప్రాక్టీస్ MCQs
                </button>
            </div>
            ...
            <div class="relative w-full lg:w-72">
                ...
                <input type="text" id="searchInput" onkeyup="filterContent()" placeholder="తెగ, జాతర లేదా సెక్షన్‌ను శోధించండి..." class="w-full pl-10 pr-4 py-2 text-sm bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all">
            </div>
        </div>
                ...
                <div class="flex flex-wrap justify-between items-center gap-4 pb-6 mb-6 border-b border-slate-100">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="px-4 py-2 rounded-xl bg-slate-900 text-white font-bold text-sm">
                            స్కోరు: <span id="quiz-score" class="text-emerald-400">0</span> / 30
                        </div>
        ...
        function switchTab(tabId) {{
            document.querySelectorAll('.tab-btn').forEach(btn => {{
                if (btn.getAttribute('data-target') === tabId) {{
                    ...
                    btn.classList.remove('bg-white', 'text-slate-600', 'bg-emerald-100', 'text-emerald-900');
                }} else {{
        ...
        function renderMCQs() {{
            ...
            appData.mcqs.forEach((mcq, idx) => {{
                ...
                mcq.options.forEach((opt, optIdx) => {{
                    optionsHtml += `
                        <button onclick="selectOption(${{idx}}, ${{optIdx}})" id="opt-btn-${{idx}}-${{optIdx}}" class="w-full text-left p-3.5 rounded-xl border border-slate-200 bg-white hover:border-emerald-500 text-sm font-medium text-slate-700 transition-all flex items-start gap-3">
                ...
                card.innerHTML = `
                    <div class="flex items-start gap-3 mb-4">
                        <span class="w-8 h-8 rounded-xl bg-emerald-600 text-white font-black flex items-center justify-center text-sm shrink-0">${{idx + 1}}</span>
